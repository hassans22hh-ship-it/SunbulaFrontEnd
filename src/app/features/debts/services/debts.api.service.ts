...
export class DebtsApiService {
  ...
  getByIdWithPayments(id: string): Observable<DebtWithPaymentsDto> { 
    return this.http.get<DebtWithPaymentsDto>(`${this.BASE}/${id}/with-payments`, this.ctx).pipe(catchError(this.handleError)); 
  }
  ...
  recordPayment(debtId: string, dto: RecordPaymentDto): Observable<DebtPaymentDto> {
    return this.http.post<DebtPaymentDto>(`${this.BASE}/${debtId}/payments`, dto, this.ctx).pipe(catchError(this.handleError));
  }
