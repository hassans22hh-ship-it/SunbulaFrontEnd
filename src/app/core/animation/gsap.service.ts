...
export class GsapService {
  ...
  /** Run GSAP animations outside NgZone to avoid unnecessary change detection */
  run<T>(fn: () => T): T {
  ...
  staggerIn(
    ...
    from: { y?: number; x?: number; scale?: number } = { y: 24 },
  ) {
    return this.run(() =>
      gsap.from(targets, {
        opacity: 0,
  ...
  slideIn(target: string | Element, dir: 'up' | 'down' | 'left' | 'right' = 'up') {
    ...
    return this.run(() =>
      gsap.from(target, { opacity: 0, duration: 0.4, ease: 'power3.out', ...map[dir] }),
    );
  }

  scaleIn(target: string | Element) {
    return this.run(() =>
      gsap.from(target, { scale: 0.88, opacity: 0, duration: 0.35, ease: 'back.out(2)' }),
    );
  }
  ...
  pulseRing(target: string | Element): gsap.core.Tween {
    return this.run(() =>
      gsap.to(target, {
        scale: 1.8,
        opacity: 0,
  ...
  pageEnter(el: Element) {
    return this.run(() =>
      gsap.from(el, { opacity: 0, y: 14, duration: 0.38, ease: 'power3.out' }),
    );
  }
